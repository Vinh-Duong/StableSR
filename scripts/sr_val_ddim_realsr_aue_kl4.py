            ...
            with model.ema_scope():
                ...
                for n in trange(niters, desc="Sampling"):
                    ...
                    for item in cur_img_list:
                        cur_image = load_img(os.path.join(opt.init_img, item),center_crop=False).to(device)
                        sr_image = F.interpolate(
									cur_image,
									size=(int(cur_image.size(-2)*opt.scale),
										  int(cur_image.size(-1)*opt.scale)),
									mode='bicubic',
									)

                        
                        # cur_image = model.get_first_stage_encoding(model.encode_first_stage(cur_image))
                    ...
                    init_latent = model.get_first_stage_encoding(model.encode_first_stage(sr_image))
                    b, c, w, h = init_latent.shape
                    print(f"init_latent size is ({w}, {h})")

                    # decode it
                    samples, _ = sampler.sample(t_enc, init_image.size(0), (3,init_image.size(2),init_image.size(3)), init_image, eta=opt.ddim_eta, verbose=False, x_T=x_T)

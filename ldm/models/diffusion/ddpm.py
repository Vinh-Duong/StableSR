...
class DDPM(pl.LightningModule):
        ...
        if sd['model.diffusion_model.input_blocks.0.0.weight'].shape[1] != self.in_channels:
            ...
            for name, param in sd.items():
                ...
                if 'model.diffusion_model.input_blocks.0.0.weight' in name:
                    ...
                    b, c, h, w = sd['model.diffusion_model.input_blocks.0.0.weight'].shape

                    print(sd['model.diffusion_model.input_blocks.0.0.weight'].shape[1])
                    # print(self.in_channels) 

                    new_layers = torch.from_numpy(np.zeros([b, self.in_channels, h, w]))

                    # print(new_layers.shape)

                    ori_channels = sd['model.diffusion_model.input_blocks.0.0.weight'].shape[1]

                    if ori_channels < self.in_channels:
                        new_layers[: ,:ori_channels,...] = param
                    else:
                        new_layers = param[:, :self.in_channels,...]
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, return_x_rec = False):
...
class LatentDiffusionSR(DDPM):
    ...
    def __init__(self,
        ...
        self.finetune_aue = finetune_aue
        ...
        # if self.cond_stage_key == 'hybird' or self.cond_stage_key == 'concat':
        ...
        if not self.frozen_unet:
            ...
            if self.finetune_dec_unet:
                self.model.eval()
                # self.model.train = disabled_train
                for name, param in self.model.named_parameters():
                    if 'diffusion_model.input_blocks' in name:
                            print(name)
                            param.requires_grad = True
        ...
        if self.distill_unet:
            ...
            print('>>>>>>>>>>>>>>>>model_teacher>>>>>>>>>>>>>>>>>>>>')
            param_list = []
            for name, params in self.model_teacher.named_parameters():
    ...
    def instantiate_first_stage(self, config):
        ...
        print('>>>>>>>>>>>>>>>>first_stage_model trainable list >>>>>>>>>>>>>>>>>>>>')
        param_list = []
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key != self.first_stage_key:
                if cond_key in ['caption', 'coordinates_bbox', 'LR_image']:
                    # xc = batch[cond_key]
                    xc = ['']*x.size(0)
                elif cond_key == 'class_label':
                    xc = batch
        ...
        if self.finetune_aue:
            out.extend([encoder_posterior, y])
    ...
    def shared_step(self, batch, **kwargs):
        if self.finetune_aue:
            x, c, gt, posterior, y = self.get_input(batch, self.first_stage_key)
            loss = self(x, c, gt, posterior, y)
        else:
            x, c, gt = self.get_input(batch, self.first_stage_key)
            loss = self(x, c, gt)
        return loss

    def forward(self, x, c, gt, posterior = None, y = None,*args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
            assert c is not None
            if self.cond_stage_trainable:
        ...
        return self.p_losses(x, c, gt, t, posterior=posterior, y=y, *args, **kwargs)
    ...
    def apply_model(self, 
        ...
        if isinstance(cond, dict):
            # hybrid case, cond is exptected to be a dict
            # print('cond is the dict')
            if not isinstance(cond['c_concat'], list):
                cond['c_concat'] = [cond['c_concat']]

            if not isinstance(cond['c_crossattn'], list):
    ...
    def p_losses(self, 
        ...
        #     cond = {'c_concat': [None],'c_crossattn': [cond]}
    
        if self.finetune_aue:
            # train encoder+decoder+logvar
            if self.conditioning_key == 'concat':
                cond = {'c_concat': [lq], 'c_crossattn': [None]}
        
            if self.conditioning_key == 'hybrid':
                cond = {'c_concat': [lq], 'c_crossattn': [cond]}

            noise = default(noise, lambda: torch.randn_like(gt))
            x_noisy = self.q_sample(x_start=gt, t=t, noise=noise)
            model_output = self.apply_model(x_noisy, t, cond)

            model_output = 1. / self.scale_factor * model_output

            model_output = self.first_stage_model.decode(model_output)

            optimizer_idx = 0
            aeloss, log_dict_ae = self.first_stage_model.loss(y, model_output, posterior, optimizer_idx = 0, global_step = self.global_step,
                                            last_layer=self.first_stage_model.get_last_layer(), split="train")
        ...
        if self.distill_unet:
            with torch.no_grad():
                if True:
                    ...
                    shape = (c, h, w)
                    batch_size = b
                    model_output_teacher, intermediates =ddim_sampler.sample(ddim_steps, batch_size,
                                                                shape,cond,verbose=False, eta=1.0, x_T=x_noisy)
                else:
                    model_output_teacher, intermediates = self.sample(cond=cond, batch_size=batch_size,
                                                        return_intermediates=True,**kwargs)
                
                # model_output_teacher = self.apply_model(x_noisy, t, cond)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if self.finetune_aue:
            z, c, z_gt, x, xrec, y, xc, encoder_posterior, gt = self.get_input(batch, self.first_stage_key,
                                            return_first_stage_outputs=True,
                                            force_c_encode=True,
                                            return_original_cond=True,
                                            bs=N)
    ...
    def configure_optimizers(self):
        ...
        if self.finetune_aue:
            # params_aue = params + list(self.first_stage_model.parameters())
    
            opt_ae = torch.optim.Adam(
                                  list(self.first_stage_model.encoder.parameters())+
...
class LatentDiffusionSRTextWTDistill(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
    ...
    def instantiate_first_stage(self, config, ckpt = None):
        ...
        # print('<<<<<<<<<<<< before >>>>>>>>>>>>>>>')
        # for name, param in self.named_parameters():
        #     if name == 'first_stage_model.encoder.conv_in.weight':
        #         print(param)
        ...
        if ckpt is not None:
            ...
            for name, param in sd.items():
                if name.startswith('first_stage_model.'):
                    ...
                    print(k)
                    sd_new[k] = param
            m, u = self.first_stage_model.load_state_dict(sd_new, strict=False)
    ...
    def instantiate_cond_stage(self, config, ckpt = None):
        if not self.cond_stage_trainable:
            if config == "__is_first_stage__":
                print("Using first stage also as cond stage.")
                self.cond_stage_model = self.first_stage_model
        ...
        if ckpt is not None:
            ...
            for name, param in sd.items():
                if name.startswith('structcond_stage_model.'):
                    ...
                    print(k)
                    sd_new[k] = param
            m, u = self.structcond_stage_model.load_state_dict(sd_new, strict=False)
    ...
    def apply_model(self, 
                    ...
                    t, 
                    cond, 
                    struct_cond = None,  # time-aware enc
                    return_ids=False,
                    model = None
                    ):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.cond_stage_key in ["image", "LR_image", "segmentation",
                                       'bbox_img'] and model.conditioning_key:  # todo check for completeness
                c_key = next(iter(cond.keys()))  # get key
                c = next(iter(cond.values()))  # get value
                assert (len(c) == 1)  # todo extend to list with more than one elem
                c = c[0]  # get element
            ...
            # apply model by loop over crops
            output_list = [model(z_list[i], t, **cond_list[i]) for i in range(z.shape[-1])]
            assert not isinstance(output_list[0],
                                  tuple)  # todo cant deal with multiple model outputs check this never happens
            ...
            if struct_cond is not None:
                cond['struct_cond'] = struct_cond
                x_recon = model(x_noisy, t, **cond)
            else:
                x_recon = model(x_noisy, t, **cond)
